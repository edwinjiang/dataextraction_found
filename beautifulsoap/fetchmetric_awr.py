...
def extrace_from_file(awrfile,line_no,result_file):
...
def extrace_from_directory(awrdir,result_file):
    ...
    line_no = 3
    for item in files:
        extrace_from_file(item,line_no,result_file)
        line_no +=1

def clear_data(*kwargs):
    try:
        result_file = kwargs["result"]
        wb = load_workbook(result_file)
        ws = wb.get_sheet_by_name("awr_metric")
        # clear excel data
        maxrow = ws.max_row
...
def extract_data(**kwargs):
    ...
    if kwargs["file"] is not None:
        ...
        extrace_from_file(awrfile,3,result_file)
    if  kwargs["directory"] is not None:
        awrdir = kwargs["directory"]
        extrace_from_directory(awrdir,result_file)
...
def _argparser():
    ...
    parser.add_argument('--r', '--result', action='store', dest='result', required=True,help='Data Result')
...
if __name__ == "__main__":
    ...
    conn_args = dict(file=parser.file,directory=parser.directory,result=parser.result)
    clear_data(**conn_args)
