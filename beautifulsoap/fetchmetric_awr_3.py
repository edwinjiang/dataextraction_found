...
def extrace_from_file(awrfile,line_no):
    with open(awrfile) as f:
        ...
        for rawdata in soup.find_all('table'):
            if re.compile('Database Instances Included In Report').search(rawdata['summary']):
                ...
                dbrelease_arr.append(tddata[6].string)
        dbrelease = dbrelease_arr[0]
        if "\n" in dbrelease_arr[0]:
            dbrelease=dbrelease_arr[0].replace('\n', '')
        #print (dbrelease)
    ...
    with open(awrfile) as f:
        ...
        if dbrelease == "12.1.0.2.0":
            with open(awrfile) as f:
                ...
                db_file_parallel_read = re.compile(r'db file parallel read')
                db_file_sequential_read = re.compile(r'db file sequential read')
                control_file_sequential_read = re.compile(r'control file sequential read')

                db_file_parallel_read_wait = []
                db_file_parallel_read_wait_time = []
                db_file_parallel_read_wait_fore_dic = {}
                ...
                try:
                    ...
                    db_file_parallel_read_wait_fore_dic["db file parallel read Waits"] = db_file_parallel_read_wait
                    db_file_parallel_read_wait_fore_dic["db file parallel read Avg Wait time"] = db_file_parallel_read_wait_time
        ...
        if dbrelease == "12.2.0.1.0":
                ...
                for rawdata in soup.find_all('table'):
                    if re.compile('This table displays foreground wait event information').search(rawdata['summary']):
                        ...
                        for trdata in trdatas:
                            ...
                            tddata = tddatas.find_all(name='td')
                            try:
                                if (db_file_parallel_read.search(tddata[1].string)):
                                    db_file_parallel_read_wait.append(tddata[2].string)
                                    db_file_parallel_read_wait_time.append(tddata[5].string)

                                if (db_file_sequential_read.search(tddata[1].string)):
                                    db_file_sequential_read_wait.append(tddata[2].string)
                                    db_file_sequential_read_wait_time.append(tddata[5].string)

                                if (control_file_sequential_read.search(tddata[1].string)):
                                    control_file_sequential_read_wait.append(tddata[2].string)
                                    control_file_sequential_read_wait_time.append(tddata[5].string)
                            except Exception as e:
                                pass
                try:
                    # print foregroud_waittime
                    ...
                    if len(db_file_sequential_read_wait_time) == 0:
                        db_file_sequential_read_wait_time.append(0)
                    db_file_sequential_read_wait_dic[
                        "db file sequential read Waits"] = db_file_sequential_read_wait
                    db_file_sequential_read_wait_dic[
                        "db file sequential read Avg Wait time"] = db_file_sequential_read_wait_time

                    if len(db_file_parallel_read_wait) == 0:
                        db_file_parallel_read_wait.append(0)
                    if len(db_file_parallel_read_wait_time) == 0:
                        db_file_parallel_read_wait_time.append('0.0ms')
                    db_file_parallel_read_wait_fore_dic[
                    "db file parallel read Waits"] = db_file_parallel_read_wait
                    db_file_parallel_read_wait_fore_dic[
                    "db file parallel read Avg Wait time"] = db_file_parallel_read_wait_time
            ...
            if dbrelease == "12.1.0.2.0":
                with open(awrfile) as f:
                    ...
                    # IO latency calculation
                    log_file_parallel_write = re.compile(r'log file parallel write')
                    control_file_parallel_write = re.compile(r'control file parallel write')
                    db_file_paralle_write = re.compile(r'db file parallel write')
            ...
            if dbrelease == "12.2.0.1.0":
                ...
                with open(awrfile) as f:
                    ...
                    db_file_parallel_read_back_wait = []
                    db_file_parallel_read_wait_back_time = []
                    db_file_parallel_read_wait_back_dic={}
                    ...
                    for rawdata in soup.find_all('table'):
                        if re.compile('This table displays background wait event information').search(
                                rawdata['summary']):
                            trdatas = rawdata.find_all(name='tr')
                            for trdata in trdatas:
                                tddatas = BeautifulSoup(str(trdata), "html.parser")
                                tddata = tddatas.find_all(name='td')
                                try:
                                    if db_file_parallel_read_wait_fore_dic["db file parallel read Waits"][0] == 0:
                                        if (db_file_parallel_read.search(tddata[1].string)):
                                            db_file_parallel_read_back_wait.append(tddata[2].string)
                                            db_file_parallel_read_wait_back_time.append(tddata[5].string)

                                    if (log_file_parallel_write.search(tddata[1].string)):
                                        log_file_parallel_write_wait.append(tddata[2].string)
                                        log_file_parallel_write_wait_time.append(tddata[5].string)

                                    if (control_file_parallel_write.search(tddata[1].string)):
                                        control_file_parallel_write_wait.append(tddata[2].string)
                                        control_file_parallel_write_wait_time.append(tddata[5].string)

                                    if (db_file_paralle_write.search(tddata[1].string)):
                                        db_file_paralle_write_wait.append(tddata[2].string)
                                        db_file_paralle_write_wait_time.append(tddata[5].string)

                                except Exception as e:
                                    pass
                    if len(db_file_parallel_read_back_wait) == 0:
                        db_file_parallel_read_back_wait.append(0)
                    if len(db_file_parallel_read_wait_back_time) == 0:
                        db_file_parallel_read_wait_back_time.append('0.0ms')
                    db_file_parallel_read_wait_back_dic[
                        "db file parallel read Waits"] = db_file_parallel_read_back_wait
                    db_file_parallel_read_wait_back_dic[
                        "db file parallel read Avg Wait time"] = db_file_parallel_read_wait_back_time
                    ...
                    if len(log_file_parallel_write_wait_time) == 0:
                        log_file_parallel_write_wait_time.append(0)
                    log_file_parallel_write_wait_dic[
                        "log file parallel write Waits"] = log_file_parallel_write_wait
                    log_file_parallel_write_wait_dic[
                        "log file parallel write Avg Wait time"] = log_file_parallel_write_wait_time
                    ...
                    if len(control_file_parallel_write_wait_time) == 0:
                        control_file_parallel_write_wait_time.append(0)
                    control_file_parallel_write_wait_dic[
                        "control file parallel write write Waits"] = control_file_parallel_write_wait
                    control_file_parallel_write_wait_dic[
                        "control file parallel write Avg Wait time"] = control_file_parallel_write_wait_time
                    ...
                    if len(db_file_paralle_write_wait_time) == 0:
                        db_file_paralle_write_wait_time.append(0)
                    db_file_paralle_write_wait_dic[
                        "db file parallel write write Waits"] = db_file_paralle_write_wait
                    db_file_paralle_write_wait_dic[
                        "db file parallel write Avg Wait time"] = db_file_paralle_write_wait_time
                    # print backgroud_waittime
                    f.close()
                    print (dbname[0]["DB Name"] + " AWR Report parsing is finished")
    ...
    try:
        ...
        ws.cell(row=line_no, column=12).value = float(result["physical read total bytes"])
        ws.cell(row=line_no, column=13).value = float(result["physical write total bytes"])
        ws.cell(row=line_no, column=14).value = float(result["Total ThroughPut"])/1024/1024/1024
        #re.findall(r'(\w*[0-9]+.+[0-9])\w*',ss)

        ws.cell(row=line_no, column=15).value = db_file_sequential_read_wait_dic["db file sequential read Waits"][0]
        #ws.cell(row=line_no, column=16).value = re.findall(r'(\w*[0-9]+.+[0-9])\w*',db_file_sequential_read_wait_dic["db file sequential read Avg Wait time"][0])[0]
        if re.compile("u").search(db_file_sequential_read_wait_dic["db file sequential read Avg Wait time"][0]):
            ws.cell(row=line_no, column=16).value = float(re.findall(r'(\w*[0-9]?.+[0-9]+)\w*',db_file_sequential_read_wait_dic["db file sequential read Avg Wait time"][0])[0])/1000
        else:
            ws.cell(row=line_no, column=16).value = float(re.findall(r'(\w*[0-9]?.+[0-9]+)\w*',db_file_sequential_read_wait_dic["db file sequential read Avg Wait time"][0])[0])

        if db_file_parallel_read_wait_fore_dic["db file parallel read Waits"][0] !=  0:
            ws.cell(row=line_no, column=17).value = db_file_parallel_read_wait_fore_dic["db file parallel read Waits"][0]
            if re.compile("u").search(db_file_parallel_read_wait_fore_dic["db file parallel read Avg Wait time"][0]):
                ws.cell(row=line_no, column=18).value = float(re.findall(r'(\w*[0-9]?.+[0-9]+)\w*',db_file_parallel_read_wait_fore_dic["db file parallel read Avg Wait time"][0])[0])/1000
            else:
                ws.cell(row=line_no, column=18).value = float(re.findall(r'(\w*[0-9]?.+[0-9]+)\w*',db_file_parallel_read_wait_fore_dic["db file parallel read Avg Wait time"][0])[0])
        else:
            ws.cell(row=line_no, column=17).value = db_file_parallel_read_wait_back_dic["db file parallel read Waits"][0]
            # ws.cell(row=line_no, column=18).value = re.findall(r'(\w*[0-9]+.+[0-9])\w*',db_file_parallel_read_wait_dic["db file parallel read Avg Wait time"][0])[0]
            if re.compile("u").search(db_file_parallel_read_wait_back_dic["db file parallel read Avg Wait time"][0]):
                ws.cell(row=line_no, column=18).value = float(re.findall(r'(\w*[0-9]?.+[0-9]+)\w*',db_file_parallel_read_wait_back_dic["db file parallel read Avg Wait time"][0])[0])/1000
            else:
                ws.cell(row=line_no, column=18).value = float(re.findall(r'(\w*[0-9]?.+[0-9]+)\w*',db_file_parallel_read_wait_back_dic["db file parallel read Avg Wait time"][0])[0])

        ws.cell(row=line_no, column=19).value = log_file_parallel_write_wait_dic["log file parallel write Waits"][0]
        #ws.cell(row=line_no, column=20).value = re.findall(r'(\w*[0-9]+.+[0-9])\w*',log_file_parallel_write_wait_dic["log file parallel write Avg Wait time"][0])[0]
        if re.compile("u").search(log_file_parallel_write_wait_dic["log file parallel write Avg Wait time"][0]):
            ws.cell(row=line_no, column=20).value = float(re.findall(r'(\w*[0-9]?.+[0-9]+)\w*',log_file_parallel_write_wait_dic["log file parallel write Avg Wait time"][0])[0])/1000
        else:
            ws.cell(row=line_no, column=20).value = float(re.findall(r'(\w*[0-9]?.+[0-9]+)\w*',log_file_parallel_write_wait_dic["log file parallel write Avg Wait time"][0])[0])

        ws.cell(row=line_no, column=21).value = db_file_paralle_write_wait_dic["db file parallel write write Waits"][0]
        #ws.cell(row=line_no, column=22).value = re.findall(r'(\w*[0-9]+.+[0-9])\w*',db_file_paralle_write_wait_dic["db file parallel write Avg Wait time"][0])[0]
        if re.compile("u").search(db_file_paralle_write_wait_dic["db file parallel write Avg Wait time"][0]):
            ws.cell(row=line_no, column=22).value = float(re.findall(r'(\w*[0-9]?.+[0-9]+)\w*', db_file_paralle_write_wait_dic["db file parallel write Avg Wait time"][0])[0])/1000
        else:
            ws.cell(row=line_no, column=22).value = float(re.findall(r'(\w*[0-9]?.+[0-9]+)\w*',db_file_paralle_write_wait_dic["db file parallel write Avg Wait time"][0])[0])
